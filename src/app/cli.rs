    ...
    Ok(CliOptions {
        ...
        config: Config {
            first_upper: matches
                ...
                .map_or(Ok(Config::default().first_upper), parse)?,
            lower_to_upper: matches
                .value_of("lower_to_upper")
                .map_or(Ok(Config::default().lower_to_upper), parse)?,
            upper_to_lower: matches
                .value_of("upper_to_lower")
                .map_or(Ok(Config::default().upper_to_lower), parse)?,
            upper_upper_to_lower: matches
                .value_of("upper_upper_to_lower")
                .map_or(Ok(Config::default().upper_upper_to_lower), parse)?,
            lower_lower_to_upper: matches
                .value_of("lower_lower_to_upper")
                .map_or(Ok(Config::default().lower_lower_to_upper), parse)?,
        },
    })
}
